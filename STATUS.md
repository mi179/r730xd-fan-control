...
Last updated: 2026-08-19（桌面版对齐 Web 外观与读数卡，EXE 重建为 v0.4.1；CI 恢复绿色）
...
| 组成部分 | 状态 |
|---|---|
| Windows GUI（`r730xd_fan/` + `main.py`） | 完成，v0.4.1。界面已全面中文化并对齐 Web：读数卡 = 进风/排风/CPU 温度 + 实时功耗，后台 60 s 一次快照；**刻意没有趋势图**（D-025） |
| 一体 EXE（含 BMC 自动安装） | v0.4.1 于 2026-08-19 重建，含中文界面与读数卡；**当前哈希见 E-037**（同日 E-036 那个更早的 0.4.1 哈希已作废）。此前 dist 里是 07-14 的旧产物（D-024） |
