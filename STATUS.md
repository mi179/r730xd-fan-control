...
Last updated: 2026-08-20（外部审计已处理并合并回主线；22 个提交已 push，CI 全绿）
...
> 上面这段列表在 2026-08-20 合并审计副本时整体过期过一次：它列的 9 项里有 5 项
> 在合并当天就已完成（副本编号 T-038/T-039/T-040/T-041/T-042，对应主线的
> T-037/T-035/T-040/T-033/T-036）。**编号在两条线上并行分配过，看到旧编号先去
> TASKS.md 核对**，别照着做第二遍。

1. **T-051** —— 轮换 iDRAC 凭据；本轮扫描后提出，做起来最快。
2. **T-014** —— 唯一还会产生错误数据的技术债（完整扫描目前只是 partial）。
3. **T-024** —— live 脚本仍靠一个 `-p` 参数拦着不被 CI 对真机执行；一个文件名就能捅穿。
4. **T-022** —— `deploy_wrt.ps1` 的真机验收；只做过静态验收。
5. **T-049 / T-050** —— 两条等业主拍板的结构性决策（exe 是否分家、预览脚本是否入库）。
   **未定之前不要替他选**，两条都问过、都没定。
6. **T-017 / T-018 / T-025 / T-006** —— 空间回收、对外发布、拆 `ui.py`、脚本文档。
7. **T-041 / T-012 / T-013** —— 已明确接受或推迟的风险，复议触发条件见 E-029。
...
- **版本漂移 → 已上机制，换了一种形态**：D-013 关掉的是**两条产品线之间**的漂移（独立版本 + 单一事实来源
  + `desktop-v*`/`web-v*` tag，bundle 脚本对 payload 硬编码副本做一致性断言，E-018）。2026-08-19
  发现它没覆盖**源码与已构建产物之间**的漂移：桌面源码改过两次，dist 里仍是旧 EXE，版本号一模一样
  （E-036）。2026-08-20 已按 D-032 在 `verify_governance.py` 增加产物陈旧检查并修复复发（E-044），
  本风险关闭。2026-08-20 审计（E-045）发现同一类问题的**文档形态**仍在：STATUS/PLAN/webapp README
  滞后于最后几个提交（已回填），`IDRAC_HOST` 默认值在 `.151`/`.111`/空之间三处矛盾（T-038 待办）。
