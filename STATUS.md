...
Last updated: 2026-08-20（全量审计：E-045 记录 15 项发现，其中 4 项已修复提交，其余转为 T-038..T-041）
...
0. **T-042** —— 审计修复的 4 个提交只在审阅副本，先合并回真实仓库再谈别的。
1. **T-014** —— 唯一还会产生错误数据的技术债（完整扫描目前只是 partial）。
2. **T-038** —— `IDRAC_HOST` 默认值三处矛盾（`.151`/`.111`/空），统一策略需先决策。
3. **T-040** —— SPEC 的启动门禁落到 app 层兜底（D-005 未配置分支把凭据发往未核验主机）。
4. **T-039** —— RMCP 探测在 core / app.py / scripts 三份实现，收敛或交叉锁一致性。
5. **T-022** —— `deploy_wrt.ps1` 的真机验收；只做过静态验收。
6. **T-024** —— live 脚本仍靠一个 `-p` 参数拦着不被 CI 对真机执行。
7. **T-041** —— `enforce_same_origin` Host 白名单 + 桌面 UI 线程阻塞命令。
8. **T-017 / T-018 / T-025 / T-006** —— 空间回收、对外发布、拆包、脚本文档。
9. **T-012 / T-013** —— 已接受的 LAN-only backlog，复议触发条件见 E-029。
